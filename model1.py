...
try:
    # Estas bibliotecas devem ter sido instaladas via requirements.txt
    import cv2
...
def convert_heic_to_jpg_if_needed(image_path):
    ...
    if image_path.lower().endswith(('.heic', '.heif')):
        try:
            ...
            # Se a conversão falhar, tenta usar o path original
    return image_path


def load_image(image_path):
    """Carrega a imagem com OpenCV e converte BGR para RGB."""
    converted_path = convert_heic_to_jpg_if_needed(image_path)

    img_bgr = cv2.imread(converted_path)
    if img_bgr is None:
        # Tenta carregar o path original como fallback, caso a conversão falhe mas o nome seja .heic
...
def create_kmeans_segmentation(img_rgb, block_size):
    """Segmentação de Cores por Blocos usando K-Means (IA de Visão Computacional)."""
    h, w, _ = img_rgb.shape
    ...
    for y in range(0, h, block_size):
        for x in range(0, w, block_size):
            ...
            if block_rgb.size > 0:
                ...
                # K-Means com 1 cluster para encontrar a cor dominante
                kmeans_block = KMeans(n_clusters=1, random_state=0, n_init=5, max_iter=20)
...
def create_plot_buffer(img_rgb, segmented_img, profile_sample, pico_lambda, pico_absorbancia):
    ...
    # Plot 2: Imagem Segmentada
    plt.subplot(2, 2, 2)
    plt.imshow(segmented_img)
    plt.title("1B. IA Decodificando Cores (Segmentação)")
    plt.axis('off')

    # Plot 3: Perfil de Intensidade
    plt.subplot(2, 2, 3)
...
async def analyze_image_endpoint(file: UploadFile = File(
    ...
    # Usamos o método file.file.read() para obter o conteúdo do UploadFile e salvar
    # O shutil.copyfileobj é ideal para lidar com arquivos grandes de forma eficiente.
    try:
        with open(temp_path, "wb") as buffer:
            shutil.copyfileobj(file.file, buffer)
        
        # 3. Executa a Análise de Imagem
        img_rgb = load_image(temp_path)
        result = perform_analysis(img_rgb)

        # 4. Prepara a Resposta JSON
        response_data = {
            "status": "success",
        ...
        raise HTTPException(status_code=400, detail="Arquivo não encontrado no disco (verifique permissões ou o formato).")
    except ValueError as e:
        raise HTTPException(status_code=400, detail=f"Erro de processamento nos dados: {e}")
    except Exception as e:
        # Erro genérico
        raise HTTPException(status_code=500, detail=f"Erro interno da API: {e}. Verifique se todas as dependências estão instaladas corretamente.")
    finally:
        # 5. Limpeza: Garante que os arquivos temporários são removidos
        try:
            ...
            # Apenas um aviso, não interrompe a API
            print(f"Aviso de limpeza: Não foi possível remover arquivo temporário: {cleanup_error}")

# O servidor Uvicorn é iniciado pelo comando 'uvicorn main:app --reload' no terminal.
# Não é necessário usar if __name__ == '__main__': aqui, mas o código está pronto.